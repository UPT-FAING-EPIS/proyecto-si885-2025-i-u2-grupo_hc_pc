...
2.  **Compatibilidad de Tipos de Datos (Texto)**: Se presentaron errores de truncamiento de datos (`String data, right truncation`) al cargar la información en la base de datos. La solución definitiva fue especificar explícitamente el tipo de dato como `TEXT` en el script de Python, lo que asegura el uso de `NVARCHAR(MAX)` en SQL Server.
3.  **Compatibilidad de Tipos de Datos (Fechas)**: Surgió un error de conversión de fechas (`Conversion failed when converting date and/or time`) debido a que los datos de la API de GitHub incluían información de zona horaria. Se resolvió eliminando esta información de las columnas de fecha en el script de Python antes de la carga.
4.  **Integridad de Datos (Claves Duplicadas)**: Se detectaron errores de violación de clave primaria (`Violation of PRIMARY KEY constraint`) al intentar insertar commits y otros registros duplicados. Esto ocurría porque la API de GitHub puede devolver la misma entidad varias veces en diferentes consultas. Se solucionó implementando un sistema de control con `sets` en Python para registrar los identificadores únicos de cada entidad (commits, issues, etc.) y asegurar que solo se procesen una vez, garantizando la integridad de los datos.
